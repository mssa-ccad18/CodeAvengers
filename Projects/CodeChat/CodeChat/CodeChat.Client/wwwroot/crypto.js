    ...
    async function encryptMessage(roomID, message, roomKey) {
        ...
        //}
        const roomKeyArray = new Uint8Array(roomKey);
    ...
    async function decryptMessage(encryptedMessage, roomID, roomKey) {
        // get the room key from IndexedDB
        const roomKey = await getDecryptedRoomKey(roomID);
        if (!roomKey) {
            throw new Error("Room key not found in IndexedDB.");
